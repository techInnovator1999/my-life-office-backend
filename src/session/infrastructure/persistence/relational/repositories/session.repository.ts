...
export class SessionRelationalRepository implements SessionRepository {
  ...
  async isValidSession(sessionId: Session['id']): Promise<boolean> {
    const isValid = await this.sessionRepository.exists({
      where: { id: sessionId as number, user: { deactivatedAt: IsNull() } },
    });
    return isValid;
  }
}
